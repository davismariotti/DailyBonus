...
public class DailyBonusItemDelay
  implements Runnable
{
  ...
  public void run()
  {
    if ((this.player.isOnline()) && (this.plugin.isEnabled()) && (!this.plugin.numEarly.containsKey(this.player.getName())))
    {
      ...
      this.player.sendMessage(DailyBonusPlayerListener.replaceColors(this.plugin.config.getString("Tier." + this.num + ".Message").replaceAll("!amount", "" + amount)));
      if (this.plugin.config.get("Tier." + this.num + ".Items") != null)
      {
        ...
        for (String itemsline : items)
        {
          ...
          if (!line[0].equals("0"))
          {
            ...
            {
              this.player.getWorld().dropItemNaturally(this.player.getEyeLocation(), is);
            }
            else
            {
      ...
      if (this.plugin.config.getBoolean("Main.Global Message is Enabled"))
      {
        for (Player p : this.plugin.getServer().getOnlinePlayers())
        {
          if (p.equals(this.player))
            continue;
          p.sendMessage(replaceColors(this.plugin.config.getString("Main.Global Message").replaceAll("!amount", "" + amount).replaceAll("!playername", "" + this.player.getDisplayName()).replaceAll("!type", "" + DailyBonus.econ.currencyNamePlural())));
        }
      }
    }
