...
public class DailyBonusItemDelay implements Runnable {
	...
	public void run() {
		if ((this.player.isOnline()) && (this.plugin.isEnabled())
				&& (!this.plugin.numEarly.containsKey(this.player.getName()))) {
			int amount = 0;
			String amt = this.plugin.config.getString("Tier." + this.num
					+ ".Economy Amount");
			if (amt.split(";").length > 1) {
				...
				amount = this.plugin.config.getInt("Tier." + this.num
						+ ".Economy Amount");
			}
			if (amount > 0) {
				EconomyResponse r;
				if (DailyBonus.econ != null) {
					r = DailyBonus.econ.depositPlayer(this.player.getName(),
							amount);
				} else {
					this.player
			...
			this.player.sendMessage(DailyBonusPlayerListener
					.replaceColors(this.plugin.config.getString(
							"Tier." + this.num + ".Message").replaceAll(
							"!amount", "" + amount)));
			if (this.plugin.config.get("Tier." + this.num + ".Items") != null) {
				List<String> items = this.plugin.config.getStringList("Tier."
						+ this.num + ".Items");
				for (String itemsline : items) {
					...
					if (!line[0].equals("0")) {
						ItemStack is = new ItemStack(
								Material.getMaterial(Integer.parseInt(line[0])),
			...
			if (this.plugin.config.get("Tier." + this.num + ".Commands") != null) {
				List<String> cmds = this.plugin.config.getStringList("Tier."
						+ this.num + ".Commands");

				for (String cmd : cmds) {
					cmd = cmd.replaceAll("!player", this.player.getName());
					this.plugin.getServer().dispatchCommand(
							this.plugin.getServer().getConsoleSender(), cmd);
				}
			}
		...
		if (this.plugin.numEarly.containsKey(this.player.getName())) {
			int num = ((Integer) this.plugin.numEarly
					.get(this.player.getName())).intValue();
			if (num <= 1) {
				this.plugin.numEarly.remove(this.player.getName());
			} else {
				this.plugin.numEarly.remove(this.player.getName());
				this.plugin.numEarly.put(this.player.getName(),
